package com.zhouweixian.instadownloader.adapters;
...
import com.zhouweixian.instadownloader.R;
...
public class InstaAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<SourceModel.ImagesBean> media;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        final SourceModel.ImagesBean m = media.get(position);

        final InstaVH inVH = (InstaVH) holder;

        if (m.isIs_video()) {
            Glide.with(inContext).load(m.getThumbnail_src()).placeholder(R.drawable.thumbnail).into(inVH.hImage);
            inVH.hImage.setVisibility(View.GONE);
            inVH.hVideoImg.setVisibility(View.VISIBLE);
            inVH.hVideoImg.setOnClickListener(view -> {
                inVH.hVideoImg.setVisibility(View.GONE);
                inVH.hVideo.setVisibility(View.VISIBLE);
                inVH.hVideo.setVideoPath(m.getVideo_url());
            ...
            inVH.hVideoImg.setVisibility(View.GONE);
            Glide.with(inContext).load(m.getDisplay_resources().get(0).getSrc()).into(inVH.hImage);
        }

        RxView.clicks(inVH.hDownload)
                .compose(new RxPermissions((FragmentActivity) inContext).ensure(Manifest.permission.WRITE_EXTERNAL_STORAGE))
                .subscribe(granted -> {
                    inDialog = new ProgressDialog(inContext);
                    inDialog.setMessage("Please wait...");
                    inDialog.setCancelable(false);
                    inDialog.show();
                    String img = (!m.isIs_video()) ? m.getDisplay_resources().get(0).getSrc() : "";
                    String vid = m.isIs_video() ? m.getVideo_url() : "";
        ...
        inVH.hRepost.setOnClickListener(view -> {
            inDialog = new ProgressDialog(inContext);
            inDialog.setMessage("Please wait...");
            inDialog.setCancelable(false);
            inDialog.show();
    ...
    private void add(SourceModel.ImagesBean r) {
        media.add(r);
        notifyItemInserted(media.size() - 1);
    }

    public void addAll(List<SourceModel.ImagesBean> media) {
        for (SourceModel.ImagesBean m : media) {
            add(m);
        }
    }

    private void remove(SourceModel.ImagesBean r) {
    ...
    private SourceModel.ImagesBean getItem() {
        return media.get(0);
    }
...
//                    filename = cursor.getString(filenameIndex);
                    Uri uri = FileProvider.getUriForFile(context,
                            context.getApplicationContext().getPackageName() + ".fileprovider", new File(Uri.parse(fileuri).getPath()));
